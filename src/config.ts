...
const paths = envPaths('webmcp', { suffix: '' });
...
export async function configureMcpClient(client: 'claude' | 'cursor' | 'cline' | 'windsurf' | 'antigravity' | 'gemini' | string) {
    ...
    switch (client) {
        ...
        case 'cursor':
            ...
            console.log('Cursor requires manual configuration. Please add the following to your Cursor MCP settings:');
        ...
        case 'cline':
            if (platform === 'darwin') {
                ...
                configPath = join(process.env.APPDATA || '', 'Code/User/globalStorage/saoudrizwan.claude-dev/settings/cline_mcp_settings.json');
            } else {
                configPath = join(homeDir, '.config/Code/User/globalStorage/saoudrizwan.claude-dev/settings/cline_mcp_settings.json');
            }
            break;
        case 'windsurf':
        ...
        case 'antigravity':
        case 'gemini':
            console.log(`${client} requires manual configuration or custom paths.`);
            console.log(`Command: node`);
            console.log(`Args: ${process.argv[1]} --mcp`);
            return;
        default:
